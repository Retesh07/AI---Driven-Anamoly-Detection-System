...
from torch.optim import lr_scheduler as TensorLROnPlateau
...
class FocalLoss(nn.Module):
    ...
    This implementation properly handles class-specific weighting by applying different
    alpha values to positive and negative classes. For binary classification:
    - Positive samples (target=1) are weighted by alphaw
    - Negative samples (target=0) are weighted by (1 - alpha)
    ...
    def forward(self, inputs, targets):
        ...
        if inputs.dim() > 1 and inputs.size(1) == 1:
            inputs = inputs.squeeze(1)  # Convert [batch, 1] to [batch]
        if targets.dim() > 1 and targets.size(1) == 1:
            targets = targets.squeeze(1)  # Convert [batch, 1] to [batch]
        ...
        # Calculate p_t for positive and negative classes
        p_t = targets * p_t + (1 - targets) * (1 - p_t)
        
        # Calculate alpha_t for class balancing
...
class ScaledDotProductAttention(nn.Module):
    """
    Scaled Dot-Product Attention mechanism with proper masking support.
    
    This implementation is kept for reference but not used in the main model.
    The model now uses nn.MultiheadAttention for better performance and integration.
    """
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None, return_attention: bool = False) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        Returns:
            If return_attention=False: Context tensor of shape (batch_size, hidden_size)
            If return_attention=True: Tuple of (context, attention_weights)
        """
...
class SuspiciousActivityLSTM(nn.Module):
    ...
    def __init__(self, 
                 ...
                 num_layers: int = 1,
                 dropout: float = 0.3,
                 use_attention: bool = True,  # Changed default to True
        ...
        Args:
            ...
            num_attention_heads: Number of attention heads if use_attention is True (default: 4)
        ...
        # Calculate output dimension after LSTM (doubled if bidirectional)
        self.lstm_output_dim = hidden_size * (2 if bidirectional else 1)
        ...
        if use_attention and num_attention_heads > 0:
            # Ensure attention heads are valid for the output dimension
            assert self.lstm_output_dim % num_attention_heads == 0, \
                f"LSTM output dimension ({self.lstm_output_dim}) must be divisible by num_attention_heads ({num_attention_heads})"
                
            self.attention = nn.MultiheadAttention(
                embed_dim=self.lstm_output_dim,
                num_heads=num_attention_heads,
                batch_first=True,
                dropout=dropout if num_attention_heads > 1 else 0
            )
            
            # Learnable parameter for attention scaling
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(self.lstm_output_dim // 2, 1)  # Output raw logits
        )
    ...
    def create_attention_mask(self, lengths: Tensor, max_len: Optional[int] = None) -> Tensor:
        """
        Create attention mask for variable-length sequences.
        
        Args:
            lengths: 1D tensor of sequence lengths (batch_size,)
            max_len: Maximum sequence length (if None, use max(lengths))
            
        Returns:
            mask: Boolean tensor of shape (batch_size, max_len) where
                  True indicates valid positions and False indicates padding
        """
        if max_len is None:
            max_len = lengths.max().item()
            
        # Create range tensor [0, 1, ..., max_len-1]
        range_tensor = torch.arange(max_len, device=lengths.device).expand(len(lengths), max_len)
        
        # Create mask where positions < lengths are True (valid)
        ...
        for name, param in self.named_parameters():
            if 'weight' in name:
                if 'lstm' in name:
                    ...
                    # Use xavier_uniform for other weights
                    nn.init.xavier_uniform_(param.data)
            elif 'bias' in name:
    ...
    def forward(
        ...
        """
        Forward pass for the LSTM model with support for variable-length sequences.
        
        Args:
            x: Input tensor of shape (batch_size, seq_len, input_size)
                or (batch_size, input_size) for single timestep
            lengths: Optional 1D tensor of sequence lengths (batch_size,).
                   If None, assumes all sequences are of full length.
            return_attention: Whether to return attention weights
                
        Returns:
            If return_attention=False: 
                Output tensor of shape (batch_size, 1) with raw logits
            If return_attention=True:
                Tuple of (logits, attention_weights)
        """
        # Add sequence dimension if input is 2D (single timestep)
        if x.dim() == 2:
        ...
        # Apply input dropout
        x_sorted = self.input_dropout(x_sorted)
        
        # Pack padded sequences
        packed_input = pack_padded_sequence(
            x_sorted, 
            lengths_sorted.cpu(),  # pack_padded_sequence needs lengths on CPU
            batch_first=True, 
            enforce_sorted=True
        )
        ...
        lstm_out, _ = pad_packed_sequence(
            packed_output, 
            batch_first=True,
            total_length=seq_len  # Ensure output has same length as input
        )
        
        # Unsort the sequences to original order
        ...
        if self.use_attention and self.attention is not None:
            ...
            # Use residual connection with learnable scale
            lstm_out = lstm_out + self.attention_scale * attn_output
            
            # Store attention weights for visualization
            self.attention_weights = attn_weights.detach().cpu().numpy()
        
        # For classification, use the output corresponding to the last valid timestep
        output = torch.stack([
            lstm_out[i, length-1] if length > 0 else lstm_out[i, 0]
        ...
        if not hasattr(self, '_logged_output_shape'):
            logger.info(f"Model output shape: {tuple(logits.shape)}")
...
class ModelTrainer:
    ...
    def __init__(self, 
        ...
        # Initialize class weights, focal alpha, and loss function
    ...
    def update_loss_function(self) -> None:
        """
        Update the loss function with current class weights and focal alpha.
        Uses Focal Loss for better handling of class imbalance.
        """
        if self.class_weights is not None:
            # Use provided focal_alpha if available, otherwise use class_weights[1]
            alpha = self.focal_alpha
            if alpha is None and self.class_weights is not None and len(self.class_weights) > 1:
                # If class_weights is a tensor, convert to Python float
                alpha = float(self.class_weights[1].item() if torch.is_tensor(self.class_weights[1]) else self.class_weights[1])
            
            # Default alpha if still None
            if alpha is None:
                alpha = 0.25  # Default value from FocalLoss
            
            # Ensure alpha is a Python float
            alpha = float(alpha)
            
            logger.info(f"Using FocalLoss with alpha={alpha:.4f}")
            ...
            self.criterion = FocalLoss(
                alpha=alpha,  # Weight for positive class
    ...
    def train_epoch(self, dataloader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, (sequences, targets) in enumerate(dataloader):
            ...
            # Ensure outputs are 1D [batch] for loss calculation
            outputs = outputs.squeeze(1) if outputs.dim() > 1 else outputs
            
            # Calculate loss - both outputs and targets should be 1D [batch]
            ...
            # Get probabilities and predictions - ensure 1D [batch] output
    ...
    def train(
        ...
        Args:
            ...
            model_name: Name of the model checkpoint file (without extension)
        ...
        best_model_path = os.path.join(checkpoint_dir, f"{os.path.splitext(model_name)[0]}_best.pth")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if val_loader is not None:
                ...
                if val_metrics['loss'] < self.best_val_loss:
                    ...
                    self.save_model(best_model_path, metrics=val_metrics)
            ...
            if checkpoint_freq > 0 and (epoch % checkpoint_freq == 0 or epoch == num_epochs):
                checkpoint_path = os.path.join(
                    checkpoint_dir, 
                    f"{os.path.splitext(model_name)[0]}_epoch{epoch:03d}.pth"
                )
                self.save_model(checkpoint_path, metrics=val_metrics)
    ...
    def load_model(self, path: str) -> Dict[str, Any]:
        ...
        # Load checkpoint without weights_only for compatibility
